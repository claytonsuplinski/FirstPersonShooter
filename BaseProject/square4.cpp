...
Square4::Square4() : Object2(){
	this->shader_index = 0;
}

void Square4::StepShader(){
	this->shader_index = ++this->shader_index % this->shaders.size();
}

bool Square4::Initialize(int x, float s, char* v, char* f)
{
	if (this->GLReturnedError("Square4::Initialize - on entry"))
		return false;
...
#pragma endregion
	...
	for (int i = 0; i < x; ++i){
		for (int j = 0; j < x; ++j){
			...
			dwn_vertex.position = vec3(m * x_axis);dwn_vertex.normal = vec3(0,1,0);
			dwn_vertex.texture_coordinate = vec2(1.0f, 0.0f);

			m = translate(m, vec3(-increment, 0.0f,0.0f));
			dwr_vertex.position = vec3(m * x_axis);dwr_vertex.normal = vec3(0,1,0);
			dwr_vertex.texture_coordinate = vec2(0.0f, 0.0f);
			
			m = translate(m, vec3(0.0f,0.0f,increment));
			nxt_vertex.position = vec3(m * x_axis);nxt_vertex.normal = vec3(0,1,0);
	...
	if (this->GLReturnedError("Square4::Initialize - on exit"))
		return false;

	return true;
}

void Square4::TakeDown(){super::TakeDown();}

void Square4::Draw(const mat4 & projection, mat4 modelview, const ivec2 & size, const float time)
{
	if (this->GLReturnedError("Square4::Draw - on entry"))
		return;
	...
	if (this->GLReturnedError("Square4::Draw - on exit"))
		return;
}
