...
bool Sphere::Initialize(int slices, float radius, char* v, char* f)
{
	...
	glVertexAttribPointer(3, 3, GL_FLOAT, GL_FALSE, sizeof(VertexAttributesPCNT), (GLvoid *) (sizeof(vec2) * 1));	// Same
...
void Sphere::Draw(const mat4 & projection, mat4 modelview, const ivec2 & size, const float time)
{
	...
	float timeDisplay = float(glutGet(GLUT_ELAPSED_TIME)) / 500.0f;
	...
	this->shaders[this->shader_index]->CommonSetup(timeDisplay, value_ptr(size), value_ptr(projection), value_ptr(modelview), value_ptr(mvp), value_ptr(nm));
